...
{
    private $table_specs = 'wpsgshcm_specs_general';
    ...
    public function motorcycleCatalogue()
    {
        try {
            $catalogue = DB::table($this->table_products)
                ...
                ->select($this->table_products . '.product_id AS id',
                         $this->table_products . '.sku',
                         $this->table_specs . '.name',
                         $this->table_specs . '.bodywork AS brand',  
                         $this->table_products . '.min_price',
                         $this->table_products . '.stock_status AS availability',
                         $this->table_specs . '.image',
                        )
                ->where($this->table_specs . '.active', "=", 1)
                ->get();
